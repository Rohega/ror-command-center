...
|-------|--------|-----------|------------|
| 2 | Actions `file` and `shell` on the project tree, status pending → complete / failed. Git commit or stash is the undo. | A failed action leaves the tree at the checkpoint. | No WebContainer. No new snapshot store. |
